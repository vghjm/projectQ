...
async function convertSubscribeType(serverType){
  ...
  return await serverType.map(subscribe => {
    return {p_id: subscribe.p_ID, pushStartTime: Moment('20200812 ' + subscribe.chatstart_time), pushEndTime: Moment('20200812 ' + subscribe.chatend_time)};
  });
}

export default async function downloadSubscribeData({jwt, debug=false}){
  let reply = {ok: false, data: null, message: ''};
  let response = await httpConnection(ADDRESS.SUBSCRIBE_LOOKUP, {jwt: jwt}, 'POST');
